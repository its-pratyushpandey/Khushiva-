  ...
  persist(
    (set, get) => ({
        ...
        webSocketService.connect(
          (response: ChatResponse) => {
            ...
            set((state) => {
              ...
              const messageExists = state.messages.some(msg => msg.id === botMessage.id)
              if (messageExists) {
                console.warn('Duplicate message detected, skipping:', botMessage.id)
                return { isLoading: false, isTyping: false }
              }
      ...
      sendMessage: async (content: string) => {
        ...
        set((state) => {
          // Check if message already exists (should not happen but safety check)
          const messageExists = state.messages.some(msg => msg.id === userMessage.id)
          if (messageExists) {
            console.warn('Duplicate user message detected, skipping:', userMessage.id)
      ...
      sendMessageViaREST: async (content: string) => {
        ...
        try {
          ...
          set((state) => {
            ...
            const messageExists = state.messages.some(msg => msg.id === botMessage.id)
            if (messageExists) {
              console.warn('Duplicate bot message detected in REST, skipping:', botMessage.id)
